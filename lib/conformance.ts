 ...
 */
export function designTokensToContract(fm: any) {
  const contract: Record<string, any> = {};
  if (fm?.colors && typeof fm.colors === "object") contract.colors = fm.colors;

  if (fm?.typography && typeof fm.typography === "object") {
    const families = new Set(), sizes = new Set();
    for (const t of (Object.values(fm.typography) as any[])) {
      if (t && typeof t === "object") {
        if (t.fontFamily) families.add(t.fontFamily);
        if (t.fontSize) sizes.add(t.fontSize);
      }
    }
    if (families.size) contract.fontFamilies = [...families];
    if (sizes.size) contract.fontSizes = [...sizes];
  }

  const flatValues = (obj) =>
    obj && typeof obj === "object" ? uniq((Object.values(obj) as any[]).filter((v) => typeof v === "string")) : [];
  const spacing = flatValues(fm?.spacing);
  const rounded = flatValues(fm?.rounded);
  if (spacing.length) contract.spacing = spacing;
  if (rounded.length) contract.borderRadius = rounded;
...
function contractValues(contract) {
  return {
    colors: uniq([
      ...
      ...(contract.colors && typeof contract.colors === "object" ? (Object.values(contract.colors) as any[]) : []),
    ].filter(Boolean)),
...
function liveValues(extract) {
  ...
  const semantic = extract.colors?.semantic
    ? (Object.values(extract.colors.semantic) as any[])
...
function colorConformance(want, have, cfg) {
  ...
  for (const c of want) {
    ...
    if (!ok) violations.push({ category: "color", token: c });
  }
...
function setConformance(category, want, have, normalize) {
  ...
  const violations = [];
  for (const v of want) {
    if (!liveSet.has(normalize(v))) violations.push({ category, token: v });
  }
...
function dimConformance(category, want, have, cfg) {
  ...
  for (const v of want) {
    ...
    if (!ok) violations.push({ category, token: v });
  }
 ...
 * @param {object} [config]
 */
export function computeConformance(contract: any, candidate: any, config: any = {}) {
