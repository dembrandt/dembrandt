...
export async function extractLogo(page, url) {
  ...
  const result = await page.evaluate(({ baseUrl, heuristicsSource }) => {
    ...
    function paintedRect(el) {
      ...
      let x = r.left + n(cs.borderLeftWidth) + n(cs.paddingLeft);
      let y = r.top + n(cs.borderTopWidth) + n(cs.paddingTop);
      let w = Math.max(0, r.width - n(cs.borderLeftWidth) - n(cs.borderRightWidth) - n(cs.paddingLeft) - n(cs.paddingRight));
      let h = Math.max(0, r.height - n(cs.borderTopWidth) - n(cs.borderBottomWidth) - n(cs.paddingTop) - n(cs.paddingBottom));
    ...
    // scanned (consensys.com, medable.com). Try the selectors in order of authority, and
    // only accept an element that is actually rendered.
