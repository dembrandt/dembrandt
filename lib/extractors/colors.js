...
export async function extractWcagStatePairs(page) {
  let rawPairs = [];
  try {
    rawPairs = await page.evaluate(() => {
      const canvas = document.createElement('canvas');
      ...
      const STATE_PATTERNS = [
        { re: /:hover\b/, state: 'hover' },
        { re: /:focus(?:-visible)?\b/, state: 'focus' },
        { re: /\[disabled\]|:disabled\b/, state: 'disabled' },
      ];

      const COLOR_PROPS = ['color', 'background-color', 'border-color', 'outline-color'];

      // Parse stylesheets for state-dependent color rules
      const cssRules = [];
      try {
        for (const sheet of document.styleSheets) {
          ...
          for (const rule of rules) {
            ...
            for (const { re, state } of STATE_PATTERNS) {
              ...
              if (Object.keys(props).length) cssRules.push({ sel, state, props });
            }
          }
        }
      } catch { /* cross-origin sheet */ }

      // For each rule, find matching elements and build pairs
      const pairs = [];
      const seen = new Set();

      for (const { sel, state, props } of cssRules) {
        // Strip the state pseudo to find base elements
        const baseSel = sel
          .replace(/:hover\b/g, '')
        ...
        let els;
        try { els = Array.from(document.querySelectorAll(baseSel)).slice(0, 5); } catch { continue; }
        ...
        for (const el of els) {
          try {
            ...
            const baseFg = toHex(base.color);
            const baseBg = findBg(el);

            for (const [prop, val] of Object.entries(props)) {
              const stateFg = prop === 'color' ? toHex(val) : baseFg;
              const stateBg = prop === 'background-color' ? toHex(val) : baseBg;
              if (!stateFg || !stateBg || stateFg === stateBg) continue;
              const key = `${state}/${[stateFg, stateBg].sort().join('/')}`;
              if (seen.has(key)) continue;
              seen.add(key);
              pairs.push({ fg: stateFg, bg: stateBg, state });
            }
          } catch { /* skip stale */ }
        }
      }
  ...
  for (const { fg, bg, state } of rawPairs) {
    try {
      const l1 = relativeLuminance(fg);
      const l2 = relativeLuminance(bg);
      if (l1 === null || l2 === null) continue;
      ...
      const darker = Math.min(l1, l2);
      const ratio = Math.round((lighter + 0.05) / (darker + 0.05) * 100) / 100;
      result.push({ fg, bg, ratio, aa: ratio >= 4.5, aaLarge: ratio >= 3, aaa: ratio >= 7, state });
    } catch { /* skip */ }
  }
