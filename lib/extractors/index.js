  ...
  try {
    ...
    for (const element of sampled) {
      try {
        ...
        // Collect hover contrast pair
        const hoverFg = afterHover.color;
        const hoverBg = afterHover.resolvedBg;
        if (hoverFg && hoverBg && (hoverFg !== beforeState.color || hoverBg !== beforeState.resolvedBg)) {
          interactiveStatePairs.push({ fg: hoverFg, bg: hoverBg, state: 'hover', tag: beforeState.tag });
        }

        if (['input', 'textarea', 'select', 'button', 'a'].includes(beforeState.tag) || beforeState.tag) {
          try {
