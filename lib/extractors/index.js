...
import { extractWcagPairs, extractWcagStatePairs } from './colors.js';
...
export async function extractBranding(url, spinner, browser, options = {}) {
  ...
  try {
    ...
    if (options.wcag) {
      ...
      try {
        const [staticPairs, statePairs] = await Promise.all([
          extractWcagPairs(page),
          extractWcagStatePairs(page),
        ]);
        wcag = staticPairs;
        const wcagStates = statePairs;
        spinner.stop();
        const passing = wcag.filter(p => p.aa).length;
        const statesFailing = wcagStates.filter(p => !p.aa).length;
        console.log(chalk.hex('#50FA7B')(`  ✓ WCAG: ${passing}/${wcag.length} pairs pass AA`) +
          (statesFailing ? chalk.hex('#FFB86C')(` · ${statesFailing} state pair(s) fail`) : ''));
        if (wcagStates.length) wcag = [...wcag, ...wcagStates.map(p => ({ ...p, source: 'state' }))];
      } catch {
        spinner.stop();
      }
    }
