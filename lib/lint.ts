...
function contrastRatio(hex1, hex2) {
  ...
  const l1 = relativeLuminance(rgb1 as any);
  const l2 = relativeLuminance(rgb2 as any);
...
function rulePrimaryContrast(result, cfg) {
  ...
  if (onWhite === null) return results;
...
function ruleButtonContrast(result, cfg) {
  ...
  for (const b of buttons) {
    ...
    if (isTransparentColor(s.backgroundColor)) continue;
    const bgHex = toHex(s.backgroundColor);
    const fgHex = toHex(s.color);
    if (!bgHex || !fgHex) continue;
    const ratio = contrastRatio(fgHex, bgHex);
    if (ratio === null) continue;
    if (!worst || ratio < worst.ratio) worst = { ratio, bg: bgHex, fg: fgHex, text: b.text };
  }

  if (!worst) return results;
 ...
 */
export function lint(result: any, config: any = {}) {
  const resolved = resolveConfig(config);
  const all = [];

  for (const [name, run] of (Object.entries(RULES) as any[])) {
    const cfg = resolved[name];
    if (cfg.level === "off") continue;
