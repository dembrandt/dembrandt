// features.ts — the single source of truth for turning a dembrandt extraction's
...
export const FEATURE_NAMES = [
    ...
    'src_text',
    'src_bg',
    // NOTE (dembrandt 0.30.0): both token features are effectively dead on current
    // extractions. They read `colors.cssVariables`, which dembrandt filters down to
    // colours absent from the palette — so a brand token that made the palette (i.e.
    // every candidate that matters) is never flagged. `--teach`, which used to emit
    // the unfiltered :root set as a sidecar, was removed. The replacement signal is
    // `detected[].isToken`; wiring it changes feature semantics, so it needs a
    // FEATURE_VERSION bump + retrain rather than a silent swap.
...
export function candidatesFrom(extraction) {
    // Candidate set = high-recall `detected` ∪ any `palette`-only entries (e.g. the
    // SVG-logo colours injected post-extraction), deduped by normalized hex. This is
    // THE place that keeps labeler, dataset and runtime seeing the same candidates.
    // Older extractions without `detected` fall back to `palette` alone.
    const detected = extraction.colors?.detected ?? [];
    const byHex = new Map();
    for (const p of [...detected, ...(extraction.colors?.palette ?? [])]) {
