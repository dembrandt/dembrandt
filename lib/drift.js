...
function compareColors(base, cand, cfg) {
  ...
  for (const bc of base) {
    ...
    if (bestIdx !== -1 && best <= cfg.colorSame) {
      ...
      used.add(bestIdx);
      changes.push({ category: "color", kind: "changed", label: bc, before: bc, after: cand[bestIdx], delta: round(best) });
      penalty += clamp01(best / cfg.colorShift);
      changed++;
    } else {
      changes.push({ category: "color", kind: "removed", label: bc, before: bc });
      penalty += 1;
      removed++;
    }
  }
  ...
  cand.forEach((cc, i) => {
    if (used.has(i)) return;
    changes.push({ category: "color", kind: "added", label: cc, after: cc });
    penalty += 0.5;
    added++;
  });

  return { changes, result: { category: "color", score: categoryScore(penalty, base.length, cand.length), changed, added, removed } };
}
...
function paletteHexes(extract) {
  return (extract.colors?.palette ?? [])
...
export function computeDrift(baseline, candidate, config = {}) {
  ...
  const parts = [
    { ...compareColors(paletteHexes(baseline), paletteHexes(candidate), cfg), w: cfg.weights.color },
    { ...compareTypography(baseline.typography?.styles ?? [], candidate.typography?.styles ?? [], cfg), w: cfg.weights.typography },
    {
      ...compareDimensions("spacing",
        (baseline.spacing?.commonValues ?? []).map((s) => s.px),
        (candidate.spacing?.commonValues ?? []).map((s) => s.px),
        cfg),
      w: cfg.weights.spacing,
    },
    {
      ...compareDimensions("radius",
        (baseline.borderRadius?.values ?? []).map((r) => r.value),
        (candidate.borderRadius?.values ?? []).map((r) => r.value),
        cfg),
      w: cfg.weights.radius,
    },
    {
      ...compareShadows(
        (baseline.shadows ?? []).map((s) => s.shadow),
        (candidate.shadows ?? []).map((s) => s.shadow)),
