...
function compareTypography(base, cand, cfg) {
  ...
  for (const b of base) {
    ...
    if (!bucket || bucket.length === 0) {
      ...
      penalty += 1; peak = Math.max(peak, 1); removed++;
      continue;
    }
    ...
    if (bd > 0) {
      ...
      penalty += sev; peak = Math.max(peak, sev);
      changed++;
    }
  }
  ...
  for (const arr of buckets.values()) {
    for (const c of arr) {
      ...
      penalty += 0.5; peak = Math.max(peak, 0.5); added++;
    }
  }
...
export function computeDrift(baseline: any, candidate: any, config: any = {}) {
  const cfg = {
