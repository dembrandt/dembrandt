...
export function writeConfig(url, result) {
  ...
  return { configPath, tokensPath, configExists, domain, tokens };
}
...
export function printInitSuccess({ configPath, tokensPath, configExists, domain, tokens }) {
  ...
  console.log(chalk.dim(`  ${rel(tokensPath)}`) + chalk.dim("  ←  extracted tokens"));
  console.log("");
