...
const DIR = ".dembrandt";
const CONFIG_FILE = `${DIR}/config.json`;
const TOKENS_FILE = `${DIR}/tokens.json`;
const SNAPSHOT_FILE = `${DIR}/snapshot.yaml`;
...
export function writeConfig(url, result, pages = []) {
  ...
  return { configPath, tokensPath, snapshotPath, configExists, domain, tokens, baseline: config.baseline };
}
...
export function printInitSuccess({ configPath, tokensPath, snapshotPath, configExists, domain, tokens, baseline }) {
  ...
  if (baseline && (baseline.includes("localhost") || baseline.includes("127.0.0.1"))) {
    console.log(chalk.yellow(`  ⚠ Baseline URL is ${baseline} — update .dembrandtrc before committing.`));
  }
  console.log("");
