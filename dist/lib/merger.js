...
function mergeColors(results) {
    ...
    for (let i = 0; i < allColors.length; i++) {
        ...
        if (pageCount > 1 && confidence === 'low')
            confidence = 'medium';
        if (pageCount > 2 && confidence === 'medium')
            confidence = 'high';
...
function mergeMotion(results) {
    ...
    results.forEach(r => {
        (r.motion?.durations || []).forEach(d => {
            if (!durationMap.has(d.value))
                durationMap.set(d.value, { ...d });
            else
                durationMap.get(d.value).count += (d.count || 1);
        });
    });
    const easingMap = new Map();
    results.forEach(r => {
        (r.motion?.easings || []).forEach(e => {
            if (!easingMap.has(e.value))
                easingMap.set(e.value, { ...e });
            else
                easingMap.get(e.value).count += (e.count || 1);
        });
    });
    const animMap = new Map();
    results.forEach(r => {
        (r.motion?.animations || []).forEach(a => {
            if (!animMap.has(a.name || a.value))
                animMap.set(a.name || a.value, { ...a });
            else
                animMap.get(a.name || a.value).count += (a.count || 1);
        });
    });
    return {
        ...base,
...
export function mergeResults(results) {
    ...
    return {
        ...
        breakpoints: [
            ...new Map(results.flatMap(r => r.breakpoints || []).map(b => [b.px, b])).values()
