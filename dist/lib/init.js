 ...
 *   "/docs/api"  -> "docs_api"
 */
export function pageSnapshotName(pathOrUrl) {
    let pathname;
    try {
...
function defaultLintRules() {
    const rules = {};
    for (const [name, { level, ...options }] of Object.entries(DEFAULT_LINT_CONFIG)) {
...
function buildTokens(result) {
    ...
    if (result.colors?.semantic) {
        ...
        for (const [key, val] of Object.entries(result.colors.semantic)) {
            ...
            if (raw)
                tokens.colors[key] = normalizeColor(raw);
        }
        tokens.colors = Object.fromEntries(Object.entries(tokens.colors).filter(([, v]) => v !== null));
        if (!Object.keys(tokens.colors).length)
    ...
    if (result.typography?.styles?.length) {
        const fontFamilies = [
            ...new Set(result.typography.styles.map((s) => s.fontFamily).filter(Boolean)),
        ];
        if (fontFamilies.length)
            tokens.fontFamilies = fontFamilies;
        const fontSizes = [
            ...new Set(result.typography.styles
                .map((s) => s.fontSize)
                .filter(Boolean)
    ...
    if (result.borderRadius?.values?.length) {
        tokens.borderRadius = [
            ...new Set(result.borderRadius.values
...
export function buildSnapshot(url, result) {
    const snapshot = {
        baseline: url,
        extractedAt: result.extractedAt,
    };
...
export function buildSnapshotYaml(snapshot) {
    let domain;
    try {
...
export function printInitSuccess({ configPath, tokensPath, snapshotPath, configExists, domain, tokens, baseline }) {
    ...
    const counts = [
        ...
        .join(", ");
    if (counts)
        console.log(chalk.dim(`  Captured: ${counts}`));
    console.log("");
