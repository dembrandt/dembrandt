    ...
    return await page.evaluate(() => {
        ...
        for (const el of candidates) {
            try {
                ...
                if (['tab', 'menuitem', 'option', 'switch', 'treeitem'].includes(role))
                    continue;
                if (el.closest('[role="tablist"], [role="menu"], [role="menubar"], nav, footer'))
                    continue;
                ...
                if (rect.height < 24 || rect.height > 100)
                    continue;
                if (rect.width < 40 || rect.width > 600)
                    continue;
                            ...
                            if (rule.selectorText) {
                                ...
                                if (matchesInput && rule.selectorText.includes(':focus')) {
                                    ...
                                    if (rule.style.backgroundColor)
                                        focusState.backgroundColor = rule.style.backgroundColor;
                                    if (rule.style.color)
                                        focusState.color = rule.style.color;
                                    if (rule.style.border)
                                        focusState.border = rule.style.border;
                                    if (rule.style.borderColor)
                                        focusState.borderColor = rule.style.borderColor;
                                    if (rule.style.boxShadow)
                                        focusState.boxShadow = rule.style.boxShadow;
                                    if (rule.style.outline)
                                        focusState.outline = rule.style.outline;
                                }
                            }
                        }
                    }
                                ...
                                if (rule.selectorText) {
                                    ...
                                    if (matchesLink && rule.selectorText.includes(':hover')) {
                                        if (!hoverState)
                                            hoverState = {};
                                        if (rule.style.color)
                                            hoverState.color = rule.style.color;
                                        if (rule.style.textDecoration)
                                            hoverState.textDecoration = rule.style.textDecoration;
                                    }
                                }
                            }
                        }
    ...
    return await page.evaluate(() => {
      ...
      `));
        ...
        badges.forEach((badge) => {
            ...
            if (width > 200 || height > 60 || fontSize > 16)
                return;
            const bg = computed.backgroundColor;
            ...
            if ((paddingTop + paddingBottom) / 2 > 16 || (paddingLeft + paddingRight) / 2 > 24)
                return;
