            ...
            try {
                for (const rule of (sheet.cssRules || [])) {
                    if (rule.media) {
                        ...
                        if (match)
                            match.forEach((m) => breakpoints.add(parseInt(m)));
                    }
                }
            }
            catch (e) { }
...
export async function detectFrameworks(page) {
    return await page.evaluate(() => {
        ...
        if (muiCount > 3)
            frameworks.push({ name: 'Material UI (MUI)', confidence: 'high', evidence: `${muiCount} MUI components` });
        // Chakra UI
        const chakraCount = countMatches('[class*="chakra-"]');
        if (chakraCount > 3)
            frameworks.push({ name: 'Chakra UI', confidence: 'high', evidence: `${chakraCount} Chakra components` });
        // Ant Design
        const antCount = countMatches('[class^="ant-"], [class*=" ant-"]');
        if (antCount > 3)
            frameworks.push({ name: 'Ant Design', confidence: 'high', evidence: `${antCount} Ant components` });
        // Vuetify
        ...
        if (polarisCount > 2)
            frameworks.push({ name: 'Shopify Polaris', confidence: 'high', evidence: `${polarisCount} Polaris components` });
        // Radix UI
        const radixCount = document.querySelectorAll('[data-radix-], [data-state]').length;
        if (radixCount > 5)
            frameworks.push({ name: 'Radix UI', confidence: 'high', evidence: `${radixCount} Radix primitives` });
        // DaisyUI
        if (tailwindEvidence.length >= 2) {
        ...
        if (countMatches('.grid-x, .grid-y, .cell') > 0 || countMatches('.button.primary, .button.secondary') > 0)
        ...
        // Bulma
        const bulmaEvidence = [];
        if (countMatches('.columns, .column') > 0 && countMatches('.column') > 2)
            bulmaEvidence.push('columns system');
        if (/\bbutton is-primary\b|\bbutton is-link\b/.test(html))
        ...
        if (bulmaEvidence.length >= 2)
            frameworks.push({ name: 'Bulma', confidence: 'high', evidence: bulmaEvidence.join(', ') });
        // Semantic UI
        ...
        if (primeCount > 5)
            frameworks.push({ name: 'PrimeReact/Vue/NG', confidence: 'high', evidence: `${primeCount} Prime components` });
        // Mantine
        const mantineCount = countMatches('[class*="mantine-"], [data-mantine]');
        if (mantineCount > 3)
            frameworks.push({ name: 'Mantine', confidence: 'high', evidence: `${mantineCount} Mantine components` });
        // Carbon Design System
        const carbonCount = countMatches('[class*="cds--"], [class*="bx--"]');
        if (carbonCount > 3)
            frameworks.push({ name: 'Carbon Design System', confidence: 'high', evidence: `${carbonCount} Carbon components` });
        // Fluent UI
        const fluentCount = countMatches('[class*="ms-"], .ms-Button, .ms-TextField');
        if (fluentCount > 5)
            frameworks.push({ name: 'Fluent UI', confidence: 'high', evidence: `${fluentCount} Fluent components` });
        // Quasar
        ...
        if (elementCount > 5)
            frameworks.push({ name: 'Element Plus/UI', confidence: 'high', evidence: `${elementCount} el- components` });
        return frameworks;
    });
}
    ...
    return await page.evaluate(() => {
        ...
        for (const el of els) {
            ...
            for (const grad of gradients) {
                if (!/^(repeating-)?(linear|radial|conic)-gradient/.test(grad))
                    continue;
...
export async function extractMotion(page) {
    ...
    const staticMotion = await page.evaluate(() => {
        function getContext(el) {
            ...
            if (tag === 'button' || role === 'button' || hint.includes('btn'))
                return 'button';
            if (tag === 'a' || role === 'link')
                return 'link';
            if (tag === 'nav' || role === 'navigation' || hint.includes('nav') || hint.includes('menu'))
                return 'nav';
            if (hint.includes('modal') || hint.includes('dialog') || hint.includes('overlay') || hint.includes('drawer'))
                return 'modal';
            if (hint.includes('card') || hint.includes('tile') || hint.includes('item'))
                return 'card';
            if (hint.includes('hero') || hint.includes('banner') || hint.includes('header'))
                return 'hero';
            if (hint.includes('tooltip') || hint.includes('popover') || hint.includes('dropdown'))
                return 'overlay';
            if (tag === 'input' || tag === 'textarea' || tag === 'select')
                return 'input';
            if (hint.includes('image') || hint.includes('img') || hint.includes('media') || hint.includes('video'))
                return 'media';
            return 'other';
        }
        ...
        for (const el of els) {
            ...
            if (rawDurations.length === 0 && (!animName || animName === 'none'))
                continue;
            ...
            if (ctx !== 'other' && rawDurations.length > 0) {
                ...
                rawProps.forEach(p => { if (p && p !== 'all' && p !== 'none') {
                    const e = cx.props.get(p) || { value: p, count: 0 };
                    e.count++;
                    cx.props.set(p, e);
                } });
            }
        ...
        for (const el of sampled) {
            try {
                ...
                if (!after)
                    continue;
                const delta = {};
                if (after.transform !== before.transform && after.transform !== 'none')
                    delta.transform = after.transform;
                if (after.opacity !== before.opacity)
                    delta.opacity = { from: before.opacity, to: after.opacity };
                if (after.bg !== before.bg)
                    delta.background = { from: before.bg, to: after.bg };
                if (after.color !== before.color)
                    delta.color = { from: before.color, to: after.color };
