...
export async function extractWithCrawl(url, spinner, browser, opts = {}) {
    const { crawl = null, sitemap = false, paths = [], silent = false, discoverLinks: _ignored, ...extractOpts } = opts;
    ...
    if (additionalUrls.length === 0) {
        ...
        result._pageResults = [{ url, result }];
        return result;
    }
    spinner.stop();
    if (!silent)
    ...
    for (let i = 0; i < additionalUrls.length; i++) {
        ...
        if (!silent)
            spinner.start(`Extracting page ${pageNum}/${total}: ${pagePath}`);
    ...
    spinner.stop();
    const extractedUrls = [url, ...additionalUrls.slice(0, allResults.length - 1)];
    const merged = mergeResults(allResults);
    merged._extractedUrls = extractedUrls;
    merged._pageResults = allResults.map((r, i) => ({ url: extractedUrls[i] ?? url, result: r }));
    return merged;
}
