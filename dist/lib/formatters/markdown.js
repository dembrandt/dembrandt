...
function buildColorRoles(result) {
    ...
    const addCandidate = (raw, source) => {
        ...
        if (!allCandidates.has(hex))
            allCandidates.set(hex, { hex, lum, sat, source });
    };
    const highConf = palette?.filter(c => c.confidence === 'high' || c.confidence === 'medium') ?? [];
    for (const c of (highConf.length ? highConf : palette ?? []))
        addCandidate(c.normalized || c.color, 'palette');
    for (const btn of result.components?.buttons ?? []) {
        const state = btn.states?.default ?? btn;
        const bg = state.backgroundColor;
        if (bg && bg !== 'transparent' && !isTransparentColor(bg))
            addCandidate(bg, 'button');
    }
    for (const link of result.components?.links ?? [])
        addCandidate(link.color, 'link');
    const roles = {};
    if (semantic && Object.values(semantic).some(Boolean)) {
        for (const [role, val] of Object.entries(semantic)) {
            ...
            if (hex)
                roles[sanitizeTokenName(role)] = hex;
        }
    }
    if (allCandidates.size) {
        ...
        for (const c of palette ?? []) {
            ...
            if (hex)
                paletteConf.set(hex, confScore[c.confidence] ?? 0);
        }
        ...
        if (!roles.primary)
            roles.primary = pick(byRank);
        if (!roles.secondary)
            roles.secondary = pick(byRank);
        if (!roles.surface)
            roles.surface = pick([...byLum].reverse());
        if (!roles['on-surface'])
            roles['on-surface'] = pick(byLum);
    }
...
function buildTypographyTokens(result) {
    ...
    for (const [index, style] of styles.entries()) {
        ...
        if (!token || !token.fontFamily && !token.fontSize)
            continue;
...
function buildSpacingTokens(result) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        if (tokens[name] !== sorted[i])
            tokens[name] = sorted[i];
    }
    return tokens;
}
...
function buildComponentTokens(result, colorRoles, roundedTokens) {
    ...
    if (button) {
        ...
        if (token)
            components['button-observed'] = token;
    }
    ...
    if (input) {
        ...
        if (token)
            components['input-observed'] = token;
    }
    return components;
}
...
function buildComponentsSection(result) {
    ...
    if (button) {
        ...
        if (btn.color ?? btn.textColor)
            parts.push(`text ${toHex(btn.color ?? btn.textColor) ?? (btn.color ?? btn.textColor)}`);
        if (btn.padding)
            parts.push(`padding ${btn.padding}`);
        if (hasVisibleBorder(btn.border))
            parts.push(`border ${btn.border}`);
        if (parts.length) {
    ...
    if (input) {
        ...
        const parts = [];
        if (hasVisibleBorder(inp.border))
            parts.push(`${inp.border.split(' ').slice(0, 2).join(' ')} border`);
        if (inp.borderRadius)
            parts.push(`${inp.borderRadius} radius`);
        if (parts.length) {
...
function typographyTokenName(style, index) {
    ...
    if (normalized.some(c => /^h1$/.test(c)))
        return 'headline-display';
    if (normalized.some(c => /^h2$/.test(c)))
        return 'headline-lg';
    if (normalized.some(c => /^h3$/.test(c)))
        return 'headline-md';
    if (normalized.some(c => /^h[4-6]$/.test(c)))
        return 'headline-sm';
    if (normalized.some(c => c === 'button'))
        return 'label-lg';
    if (normalized.some(c => c === 'a'))
        return 'label-md';
    if (normalized.some(c => c === 'p' || c === 'div'))
        return 'body-md';
    return `text-${index + 1}`;
}
...
function firstButton(buttons) {
    ...
    return normalized.find(button => button.backgroundColor && !isTransparentColor(button.backgroundColor) ||
        positiveDimension(button.borderRadius) ||
        meaningfulPadding(button.padding) ||
        hasVisibleBorder(button.border)) ?? normalized.find(button => button.backgroundColor ||
        button.color ||
        button.textColor ||
        button.padding ||
        button.borderRadius ||
        hasVisibleBorder(button.border)) ?? null;
}
...
function isTransparentColor(value) {
    ...
    if (slashParts.length === 2)
        return isZeroAlpha(slashParts[1]);
