 ...
 */
import { buildDembrandtExtension, EXTENSION_KEY } from '../version.js';
/**
...
function exportTypography(typography) {
    ...
    typography.styles.slice(0, 10).forEach((style, index) => {
        ...
        const fontSize = toW3CDimension(style.size);
        if (!fontSize)
            return;
        ...
        textStyles[name] = {
            ...
            $value: {
                ...
                letterSpacing: (style.letterSpacing && toW3CDimension(style.letterSpacing)) || { value: 0, unit: 'px' }
            }
        };
    });
...
function exportSpacing(spacing) {
    ...
    spacing.commonValues.slice(0, 12).forEach((value, index) => {
        const dim = toW3CDimension(value.px || value);
        if (!dim)
...
function exportBorderRadius(borderRadius) {
    ...
    borderRadius.values
        ...
        .slice(0, 6)
        .forEach((entry, index) => {
        const dim = toW3CDimension(entry.value);
        if (!dim)
...
function exportBorders(borders) {
    ...
    borders.combinations
        ...
        if (combo.width && !seenWidths.has(combo.width)) {
            const dim = toW3CDimension(combo.width);
            if (dim) {
...
function exportShadows(shadows) {
    ...
    shadows
        ...
        const dim = (v) => toW3CDimension(v) || zero;
        shadowTokens[name] = {
            $type: 'shadow',
            $value: {
                offsetX: dim(parts[0]),
...
export function toW3CFormat(extractionResult) {
    ...
    w3cTokens.$extensions = {
        [EXTENSION_KEY]: buildDembrandtExtension(extractionResult),
    };
    // Export colors
